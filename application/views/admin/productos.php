                ...
                <div class="modal-body">
                    <form action="<?php echo base_url(); ?>productos/save" class="form form-horizontal" name="frm-productos" id="frm-productos" method="post" enctype="multipart/form-data" target="myframe">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <input class="form-control" type="file" name="lista" id="lista" data-group="lista" onchange="productos.readerFile();" />
                            </div>
                        </div>
                    </form>
                </div>
...
var productos = {
    ...
    init: function()
    {
        $('#lista').filestyle({
            ...
            //size: "sm",
            badge: false,
            placeholder: "Imagen de "+productos.size['lista'], 
        });
    ...
    editar: function(_id)
    {
        $.post(PATH+'admin/productos/get/'+_id, {}, function(response, textStatus, xhr) {
            if (response.result=="success") {
                ...
                productos.loadImage(producto.lista, '#img-lista');
                // productos.loadImage(producto.banner, '#img-banner');
                // productos.loadImage(imagenes, '#img-preview');
                //console.log(imagenes);
                if (producto.lista) {
                    uri = producto.lista.split('/');
                    $('#lista').attr({
                        // 'onchange': "productos.changeImage("+producto.id_lista+", "+producto.id+")", 
                        'data-file': uri[uri.length-1],
                        'data-producto': producto.id,
                        'data-lista': producto.id_lista
                    });
                }
