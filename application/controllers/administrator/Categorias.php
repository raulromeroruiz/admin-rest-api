...
class Categorias extends CI_Controller {
	...
	{
		$categoria = $this->categorias->get_categoria($id);
		$imagenes = $this->categorias->get_banners($id);
	...
	public function save() 
	{
		...
		$id = $datos['id'];
		if (empty($datos['id'])) {
			$query = $this->manager->insert("categorias", $datos);
			$id = $this->db->insert_id();
		}
		else {
			$query = $this->manager->update("categorias", $datos, $datos['id']);
		}
		if ( $query ) {
			if (!empty($_FILES)) {
				...
				$this->categorias->save_photos($fotos, $data);
			}
	...
	public function delete()
	{
		...
		if ($this->manager->delete('categorias', $id)) {
	...
	{
		$oficinas = $this->categorias->get_offices_admin($id);
		$tipo_categorias = $this->manager->get_fields('tipo_inmuebles', array('id', 'nombre'));

		$data = array(
			'oficinas' => $oficinas, 
			'categoria' => $id, 
			'tipo_categorias' => $tipo_categorias, 
		);
		...
		switch ($id) {
			case 'save':
				...
				if ( $query ) {
					...
					$fotos['imagen3d'] = $this->upload('imagen3d', 'planos', $datos['categoria']);
					$fotos['plano'] = $this->upload('plano', 'planos', $datos['categoria']);
			...
			default:
				$oficina = $this->categorias->get_detailoffice($id);
