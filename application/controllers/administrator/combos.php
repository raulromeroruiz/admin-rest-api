...
class Combos extends CI_Controller {
	...
	public function __construct()
   	{
    	parent::__construct();
        $this->load->model('combos_model', 'combos');
        $this->load->model('manager_model', 'manager');
   	}

	public function index()
	{
		$combos = $this->combos->all();
		...
		$this->load->view('admin/combos', $data);
		$this->load->view('admin/footer');
	}

	public function get($id)
	{
		$combo = $this->combos->get_combo($id);
		$imagenes = $this->combos->get_banners($id);
	...
	public function save() 
	{
		...
		// $datos['url'] = $this->tools->urlfriendly($datos['combo']."-".$datos['nombre']);
		// $datos['entrega'] = (isset($datos['entrega'])) ? 1:0;
		$datos['estado'] = 1;

		$id = $datos['id'];
		if (empty($datos['id'])) {
			$query = $this->manager->insert("combos", $datos);
			$id = $this->db->insert_id();
		}
		else {
			$query = $this->manager->update("combos", $datos, $datos['id']);
		}
		if ( $query ) {
			...
			if (!empty($_FILES)) {
				...
				$this->combos->save_photos($fotos, $data);
				//$this->combos->save_banners($banners, $data);
	...
	public function delete()
	{
		...
		if ($this->manager->delete('combos', $id)) {
	...
	{
		$oficinas = $this->combos->get_offices_admin($id);
		$tipo_combos = $this->manager->get_fields('tipo_inmuebles', array('id', 'nombre'));

		$data = array(
			'oficinas' => $oficinas, 
			'combo' => $id, 
			'tipo_combos' => $tipo_combos, 
		);
		...
		switch ($id) {
			case 'save':
				...
				if ( $query ) {
					...
					$fotos['imagen3d'] = $this->upload('imagen3d', 'planos', $datos['combo']);
					$fotos['plano'] = $this->upload('plano', 'planos', $datos['combo']);
			...
			default:
				$oficina = $this->combos->get_detailoffice($id);
